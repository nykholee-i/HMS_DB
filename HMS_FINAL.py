...
def create_signup_box():
    ...
    # Add space at the top before the login information
    top_space_label = tk.Label(signup_box, text="Sign Up", font=("Arial", 16), bg="white")
    top_space_label.pack(pady=10)
    ...
    age_entry = tk.Entry(signup_box, textvariable=age_var, width=5, font=("Arial", 12))
...
def create_admin_login_box():
    ...
    top_space_label = tk.Label(admin_login_box, text="Admin Login", font=("Arial", 16), bg="white")
    top_space_label.pack(pady=10)
...
def create_admin_window(cursor, mydb):
    ...
    delete_button = tk.Button(admin_frame, text="Delete Selected Appointment", command=delete_selected_appointment, bg="red", fg="white", relief="raised", font=("Helvetica", 12), width=15)
...
def create_doctors_frame(parent):
    ...
    # Set column widths
    column_widths = [200, 50, 70, 120, 150, 100, 120, 220, 110]
...
def create_tcnr_info_frame(parent):
    ...
    tcnr_info_label = tk.Label(header_frame, text="TCNR's Hospital Information", font=("Arial", 20, "bold"), bg="#27ae60", fg="white")  # Adjust text color
...
def appointment_window(full_name, age, sex):
    ...
    background_image = Image.open("D:/Nykhole's Files - Copy/[ALL] BSU - SECOND YEAR/FILES/ADV. COMPROG/HMS_DB/signup.png")
    background_image = background_image.resize((500,650), Image.Resampling.NEAREST)
    background_photo = ImageTk.PhotoImage(background_image)
